...
/**
 * This class is based on the ConfigurableEmitter class delivered with Slick2D.
 * It uses all its code and an object from the class OneTimeEmitter can easily be converted to a ConfigurableEmitter and the other way
 * round.
 * 
 * The one difference between this class and the ConfigurableEmitter is that it does not spawn its particles based on a time delay but
 * you can call the spawn method in order to create some particles. This can be usefull in order to create some nice effects.
 * 
 * 
 * @author kevin, Kemiren
 *
 */

@SuppressWarnings({ "unchecked", "rawtypes", "unused" })
public class OneTimeEmitter implements ParticleEmitter{
	...
	public OneTimeEmitter(String name) {
		this.name = name;
	...
	private static Field particleX, particleY;
	static Field[] allFieldsOneTime, allFieldsConfig;
	static{
		try {
			particleX = Particle.class.getDeclaredField("x");
			particleX.setAccessible(true);
			
			particleY = Particle.class.getDeclaredField("y");
			particleY.setAccessible(true);
		} catch (NoSuchFieldException | SecurityException e) {
			e.printStackTrace();
		}
		
		LinkedList<Field> configFieldsList = new LinkedList<Field>();
		Collections.addAll(configFieldsList, ConfigurableEmitter.class.getFields());
		Collections.addAll(configFieldsList, ConfigurableEmitter.class.getDeclaredFields());
		allFieldsConfig = configFieldsList.toArray(new Field[]{});
		for(Field f : allFieldsConfig){
			f.setAccessible(true);
		}
		LinkedList<Field> oneTimeFieldsList = new LinkedList<Field>();
		Collections.addAll(oneTimeFieldsList, OneTimeEmitter.class.getFields());
		Collections.addAll(oneTimeFieldsList, OneTimeEmitter.class.getDeclaredFields());
		allFieldsOneTime = oneTimeFieldsList.toArray(new Field[]{});
		for(Field f : allFieldsOneTime){
			f.setAccessible(true);
		}
...
//		particle.y += adjusty;
	 ...
	 */
//	public OneTimeEmitter duplicate() {
//		OneTimeEmitter theCopy = null;
//		try {
//			ByteArrayOutputStream bout = new ByteArrayOutputStream();
//			ParticleIO.saveEmitter(bout, convertOneTimeToConfigurable(this));
//			ByteArrayInputStream bin = new ByteArrayInputStream(bout.toByteArray());
//			theCopy = convertConfigurableToOneTime(ParticleIO.loadEmitter(bin));
//		} catch (IOException e) {
...
//	}
	 ...
	 * @author void
	 */
	public interface Value extends ConfigurableEmitter.Value{
		/**
	...
	public OneTimeEmitter duplicate() {
		OneTimeEmitter theCopy = null;
		ByteArrayOutputStream bout = new ByteArrayOutputStream();
		OneTimeEmitterIO.saveEmitter(bout, this);
		ByteArrayInputStream bin = new ByteArrayInputStream(bout.toByteArray());
		theCopy = OneTimeEmitterIO.loadEmitter(bin);
		return theCopy;
	}
}
